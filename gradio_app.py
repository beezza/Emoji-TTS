...
def _scan_configs() -> list[str]:
    ...
    return sorted(str(p) for p in CONFIGS_DIR.glob("*.yaml")) + sorted(
        str(p) for p in CONFIGS_DIR.glob("*.yml")
    )
...
def _scan_speakers() -> list[str]:
    ...
    return ["（使用しない）"] + sorted(
        d.name for d in SPEAKERS_DIR.iterdir() if d.is_dir() and (d / "ref.pt").exists()
    )
...
def _run_generation(
    ...
    if _use_multiline:
        ...
        for li, line in enumerate(lines):
            ...
            all_detail_lines.append(f"[行 {li + 1}] seed={result.used_seed}  text={line[:40]!r}")
    ...
    # ── 通常モード（デフォルト） ────────────────────────────────────
        ...
        for i in range(num_candidates):
            ...
            all_detail_lines.append(f"[候補 {i + 1}] seed={result.used_seed}  saved={out_path}")
            for msg in result.messages:
                all_detail_lines.append(f"  {msg}")
...
def _read_train_log() -> str:
    ...
    if eta and proc is not None and proc.poll() is None:
        ...
        if h > 0:
            ...
            eta_str = f"{s}秒"
        text += f"\n\n--- ETA: 残り約 {eta_str}  (step={step}, {speed:.3f} steps/sec) ---"
    return text
...
def _build_dataset_command(
    ...
    if mode == "スライスのみ":
        ...
        if str(whisper_backend).strip() and whisper_backend != "faster-whisper":
            cmd += ["--whisper-backend", str(whisper_backend).strip()]
            # transformers選択時はカスタムモデルを使用（指定されている場合）
            if str(transformers_model).strip():
                # --modelパラメータを上書き
                if "--model" in cmd:
                    idx = cmd.index("--model")
                    cmd[idx + 1] = str(transformers_model).strip()
                else:
                    cmd += ["--model", str(transformers_model).strip()]
...
def build_ui() -> gr.Blocks:
    ...
    default_config = next(
        (c for c in initial_configs if DEFAULT_CONFIG in c),
    ...
    with gr.Blocks(title="Irodori-TTS GUI") as demo:
        ...
        with gr.Tabs():
            ...
            with gr.Tab("🔊 推論"):
                ...
                with gr.Accordion(
                    "⬇️ HuggingFace からモデルをダウンロード", open=not bool(initial_checkpoints)
                ):
                    with gr.Row():
                ...
                with gr.Accordion(
                    "🎤 参照音声（省略するとno-referenceモードで生成）", open=False
                ) as infer_ref_accordion:
                ...
                with gr.Accordion(
                    "🎨 Caption (Voice Design)", open=False, visible=False
                ) as caption_vd_accordion:
                ...
                hf_dl_btn.click(
                    _download_from_hf, inputs=[hf_repo_id], outputs=[infer_checkpoint, hf_dl_status]
                )
                infer_refresh_btn.click(
                    lambda: gr.Dropdown(
                        choices=_scan_checkpoints(), value=(_scan_checkpoints() or [None])[-1]
                    ),
                ...
                load_model_btn.click(
                    ...
                    outputs=[
                        ...
                        infer_codec_repo,
                        infer_lora_compat_status,
                        caption_vd_accordion,
                        infer_ref_accordion,
                    ],
                )
                unload_model_btn.click(
                    _clear_runtime_cache_ui,
                    outputs=[model_status, caption_vd_accordion, infer_ref_accordion],
                )
            ...
            with gr.Tab("📂 Prepare Manifest"):
                ...
                def _on_pm_prepare_mode_change(mode: str):
                    ...
                    return (
                        gr.update(
                            visible=not is_voice,
                            interactive=not is_voice,
                            value="" if is_voice else None,
                        ),
                ...
                def _auto_fill_columns(file_path: str, mode: str):
                    ...
                    audio_choices = ["audio"] + [
                        h for h in headers if h not in {"audio", "file_name"}
                    ]
                    ...
                    spk_choices = [""] + [
                        h for h in headers if h not in {"file_name", "audio", text_guess}
                    ]
                    cap_choices = [""] + [
                        h for h in headers if h not in {"file_name", "audio", text_guess}
                    ]
                ...
                pm_cmd_preview = gr.Textbox(
                    label="📋 実行コマンドプレビュー", interactive=False, lines=3
                )
            ...
            with gr.Tab("🏋️ 学習"):
                ...
                with gr.Accordion("💾 プリセット管理（configs/ フォルダ）", open=True):
                    ...
                    preset_status = gr.Textbox(
                        label="プリセット操作結果", interactive=False, lines=1
                    )
                ...
                with gr.Row():
                    num_gpus = gr.Slider(
                        label="GPU数（1=単体, 複数=DDP）", minimum=1, maximum=8, value=1, step=1
                    )
                ...
                with gr.Accordion("⚙️ バッチ・精度設定", open=True):
                    gr.Markdown(
                        "*バッチサイズと勾配蓄積ステップの積が実効バッチサイズになります。*"
                    )
                    with gr.Row():
                        t_batch_size = gr.Slider(
                            label="バッチサイズ（GPUメモリに合わせて調整）",
                            minimum=1,
                            maximum=64,
                            value=_v("batch_size", 4),
                            step=1,
                        )
                        t_grad_accum = gr.Slider(
                            label="勾配蓄積ステップ数（実効バッチを増やす）",
                            minimum=1,
                            maximum=32,
                            value=_v("gradient_accumulation_steps", 2),
                            step=1,
                        )
                        t_num_workers = gr.Slider(
                            label="DataLoaderワーカー数",
                            minimum=0,
                            maximum=16,
                            value=_v("num_workers", 4),
                            step=1,
                        )
                    with gr.Row():
                        t_persistent_workers = gr.Checkbox(
                            label="ワーカーの永続化（起動高速化）",
                            value=_v("dataloader_persistent_workers", True),
                        )
                        t_prefetch_factor = gr.Slider(
                            label="プリフェッチ係数",
                            minimum=1,
                            maximum=8,
                            value=_v("dataloader_prefetch_factor", 2),
                            step=1,
                        )
                        t_allow_tf32 = gr.Checkbox(
                            label="TF32を許可（Ampere以降GPU向け高速化）",
                            value=_v("allow_tf32", True),
                        )
                        t_compile_model = gr.Checkbox(
                            label="torch.compileを使用（初回遅延あり）",
                            value=_v("compile_model", False),
                        )
                        t_precision = gr.Dropdown(
                            label="学習精度",
                            choices=["bf16", "fp32", "fp16"],
                            value=_v("precision", "bf16"),
                        )

                with gr.Accordion("🔧 オプティマイザ設定", open=True):
                    gr.Markdown("*Muon: 行列重み向けの高性能オプティマイザ。AdamW: 汎用的で安定。*")
                    with gr.Row():
                        t_optimizer = gr.Dropdown(
                            label="オプティマイザ",
                            choices=["muon", "adamw", "lion", "ademamix", "sgd"],
                            value=_v("optimizer", "muon"),
                        )
                        t_learning_rate = gr.Number(label="学習率", value=_v("learning_rate", 3e-4))
                        t_weight_decay = gr.Number(
                            label="重み減衰（L2正則化）", value=_v("weight_decay", 0.01)
                        )
                    with gr.Row():
                        t_muon_momentum = gr.Number(
                            label="Muonモメンタム（Muon使用時のみ有効）",
                            value=_v("muon_momentum", 0.95),
                        )
                        t_adam_beta1 = gr.Number(
                            label="Adam β1（AdamW使用時）", value=_v("adam_beta1", 0.9)
                        )
                        t_adam_beta2 = gr.Number(
                            label="Adam β2（AdamW使用時）", value=_v("adam_beta2", 0.999)
                        )
                        t_adam_eps = gr.Number(
                            label="Adam ε（AdamW使用時）", value=_v("adam_eps", 1e-8)
                        )

                with gr.Accordion("📈 学習率スケジューラ", open=True):
                    gr.Markdown(
                        "*wsd: warmup→stable→decay の3段階スケジュール。cosine: コサインアニーリング。*"
                    )
                    with gr.Row():
                        t_lr_scheduler = gr.Dropdown(
                            label="スケジューラ種別",
                            choices=["wsd", "cosine", "none"],
                            value=_v("lr_scheduler", "wsd"),
                        )
                        t_warmup_steps = gr.Number(
                            label="ウォームアップステップ数",
                            value=_v("warmup_steps", 300),
                            precision=0,
                        )
                        t_stable_steps = gr.Number(
                            label="安定期ステップ数（wsdのみ）",
                            value=_v("stable_steps", 2100),
                            precision=0,
                        )
                        t_min_lr_scale = gr.Number(
                            label="最小学習率スケール比率（0〜1）", value=_v("min_lr_scale", 0.01)
                        )

                with gr.Accordion("🔢 学習ステップ・テキスト設定", open=True):
                    with gr.Row():
                        t_max_steps = gr.Number(
                            label="最大学習ステップ数", value=_v("max_steps", 3000), precision=0
                        )
                        t_max_text_len = gr.Number(
                            label="テキスト最大トークン長",
                            value=_v("max_text_len", 256),
                            precision=0,
                        )
                        t_max_latent_steps = gr.Number(
                            label="ラテント最大フレーム数",
                            value=_v("max_latent_steps", 750),
                            precision=0,
                        )
                        t_fixed_target_latent_steps = gr.Number(
                            label="固定ターゲットラテント長",
                            value=_v("fixed_target_latent_steps", 750),
                            precision=0,
                        )
                        t_fixed_target_full_mask = gr.Checkbox(
                            label="固定ターゲット全マスク", value=_v("fixed_target_full_mask", True)
                        )

                with gr.Accordion("🎲 Conditioningドロップアウト・タイムステップ", open=False):
                    gr.Markdown(
                        "*ドロップアウト率を高めると過学習防止。小データセットでは0.1〜0.2推奨。*"
                    )
                    with gr.Row():
                        t_text_dropout = gr.Slider(
                            label="テキスト条件ドロップアウト率（0=無効）",
                            minimum=0.0,
                            maximum=0.5,
                            value=_v("text_condition_dropout", 0.15),
                            step=0.01,
                        )
                        t_speaker_dropout = gr.Slider(
                            label="話者条件ドロップアウト率（0=無効）",
                            minimum=0.0,
                            maximum=0.5,
                            value=_v("speaker_condition_dropout", 0.15),
                            step=0.01,
                        )
                        t_timestep_stratified = gr.Checkbox(
                            label="タイムステップ層化サンプリング（安定化に有効）",
                            value=_v("timestep_stratified", True),
                        )

                with gr.Accordion("💾 グラフ更新・チェックポイント保存設定", open=False):
                    with gr.Row():
                        t_log_every = gr.Number(
                            label="グラフ描画間隔（ステップ数）",
                            value=_v("log_every", 10),
                            precision=0,
                            info="この間隔でloss/lrをログ出力→グラフに反映。ファイル保存とは無関係。",
                        )
                        t_save_every = gr.Number(
                            label="チェックポイント保存間隔（ステップ数）",
                            value=_v("save_every", 100),
                            precision=0,
                        )

                with gr.Accordion("📊 Weights & Biases 設定", open=False):
                    gr.Markdown(
                        "*wandb_enabledをオンにするとクラウドでリアルタイム学習曲線を確認できます。*"
                    )
                    with gr.Row():
                        t_wandb_enabled = gr.Checkbox(
                            label="W&B を有効化", value=_v("wandb_enabled", False)
                        )
                        t_wandb_project = gr.Textbox(
                            label="W&B プロジェクト名", value=_v("wandb_project", "") or ""
                        )
                        t_wandb_run_name = gr.Textbox(
                            label="W&B 実行名（省略可）", value=_v("wandb_run_name", "") or ""
                        )

                with gr.Accordion("✅ バリデーション設定", open=False):
                    gr.Markdown(
                        "*valid_ratioを0より大きくするとバリデーションlossを監視できます。early_stoppingには必須。*"
                    )
                    with gr.Row():
                        t_valid_ratio = gr.Slider(
                            label="バリデーション分割比率（0=無効）",
                            minimum=0.0,
                            maximum=0.5,
                            value=_v("valid_ratio", 0.0),
                            step=0.01,
                        )
                        t_valid_every = gr.Number(
                            label="バリデーション実行間隔（ステップ数）",
                            value=_v("valid_every", 100),
                            precision=0,
                        )

                with gr.Accordion("🔀 オプション機能", open=False):
                    gr.Markdown(
                        "*Early Stoppingはvalid_ratio > 0 のときのみ有効。EMAは推論品質向上に有効。*"
                    )
                    with gr.Row():
                        t_early_stopping = gr.Checkbox(
                            label="Early Stopping を有効化（valid lossが改善しなくなったら自動停止）",
                            value=False,
                        )
                        t_es_patience = gr.Number(
                            label="Early Stopping: 悪化を許容する回数", value=3, precision=0
                        )
                        t_es_min_delta = gr.Number(
                            label="Early Stopping: カウント最小悪化量", value=0.01
                        )
                    with gr.Row():
                        t_use_ema = gr.Checkbox(
                            label="EMA（指数移動平均）を有効化（推論品質向上）", value=False
                        )
                        t_ema_decay = gr.Number(
                            label="EMA減衰率（0に近いほど追従速度が速い）", value=0.9999
                        )
                    t_seed = gr.Number(
                        label="乱数シード（再現性のために固定推奨）",
                        value=_v("seed", 0),
                        precision=0,
                    )

                gr.Markdown("### 📋 実行コマンドプレビュー")
                train_cmd_preview = gr.Textbox(
                    label="コマンドライン（確認用）", interactive=False, lines=3
                )
                ...
                train_log_text = gr.Textbox(
                    label="学習ログ（末尾200行）",
                    interactive=False,
                    lines=15,
                    max_lines=15,
                    elem_id="train_log_text",
                )
...
""")
                ...
                if _PANDAS_AVAILABLE:
                    gr.Markdown(
                        "*ログ・グラフは自動更新されます（手動更新ボタンでも即時反映可）。*"
                    )
                    ...
                    train_log_refresh_btn.click(
                        _do_refresh, outputs=[train_log_text, loss_plot, lr_plot]
                    )
                    ...
                    metrics_text = gr.Textbox(
                        label="メトリクス（step / loss / lr）", interactive=False, lines=6
                    )
                    ...
                    def _do_refresh_nopd():
                        ...
                        if df is None:
                            ...
                            lines = [
                                f"step={int(r['step'])}  loss={r['loss']:.4f}  lr={r['lr']:.2e}"
                    ...
                    train_log_refresh_btn.click(
                        _do_refresh_nopd, outputs=[train_log_text, metrics_text]
                    )
                ...
                _train_cfg_inputs = [
                    ...
                    t_text_dropout,
                    t_speaker_dropout,
                    t_timestep_stratified,
                    t_max_latent_steps,
                    t_fixed_target_latent_steps,
                    t_fixed_target_full_mask,
                    t_log_every,
                    t_save_every,
                    t_wandb_enabled,
                    t_wandb_project,
                ...
                for comp in [
                    ...
                    comp.change(
                        _update_train_cmd, inputs=_train_exec_inputs, outputs=[train_cmd_preview]
                    )
                ...
                def _load_preset(config_path: str):
                    ...
                    return (
                        ...
                        g("num_workers", 4),
                        g("dataloader_persistent_workers", True),
                        g("dataloader_prefetch_factor", 2),
                        g("allow_tf32", True),
                        g("compile_model", False),
                        g("precision", "bf16"),
                        g("optimizer", "muon"),
                        g("muon_momentum", 0.95),
                        g("learning_rate", 3e-4),
                        g("weight_decay", 0.01),
                        g("adam_beta1", 0.9),
                        g("adam_beta2", 0.999),
                        g("adam_eps", 1e-8),
                        ...
                        g("max_steps", 3000),
                        g("max_text_len", 256),
                        g("text_condition_dropout", 0.15),
                        g("speaker_condition_dropout", 0.15),
                        g("timestep_stratified", True),
                        g("max_latent_steps", 750),
                        g("fixed_target_latent_steps", 750),
                ...
                preset_dropdown.change(
                    _load_preset, inputs=[preset_dropdown], outputs=_preset_outputs
                )
                ...
                train_manifest_refresh.click(
                    lambda: gr.Dropdown(
                        choices=_scan_manifests(), value=(_scan_manifests() or [None])[-1]
                    ),
            ...
            with gr.Tab("🚀 LoRA学習"):
                ...
                with gr.Row():
                    ...
                    lora_output_dir = gr.Textbox(
                        ...
                        value="",
                        placeholder=str(LORA_DIR / "{run_name}"),
                        scale=2,
                    )
                ...
                with gr.Accordion("🔧 LoRA設定", open=True):
                    with gr.Row():
                        lora_rank = gr.Slider(
                            label="LoRAランク", minimum=1, maximum=128, value=16, step=1
                        )
                ...
                with gr.Accordion("🔄 Resume設定", open=False):
                    lora_resume_enabled = gr.Checkbox(
                        label="Resume（既存LoRAから再開）", value=False
                    )
                    with gr.Row():
                ...
                with gr.Accordion("⚙️ 学習パラメータ", open=True):
                    ...
                    with gr.Row():
                        ...
                        lora_warmup_steps = gr.Number(
                            label="ウォームアップステップ", value=0, precision=0
                        )
                    with gr.Row():
                        lora_max_steps = gr.Number(
                            label="最大学習ステップ", value=1000, precision=0
                        )
                ...
                with gr.Accordion("✅ バリデーション設定", open=False):
                    with gr.Row():
                        ...
                        lora_valid_every = gr.Number(
                            label="バリデーション実行間隔", value=100, precision=0
                        )

                # ── Early Stopping ──
                with gr.Accordion("🛑 Early Stopping設定", open=False):
                    with gr.Row():
                        lora_early_stopping = gr.Checkbox(
                            label="Early Stoppingを有効化", value=False
                        )
                ...
                for comp in [
                    ...
                    comp.change(
                        _update_lora_cmd, inputs=_lora_exec_inputs, outputs=[lora_cmd_preview]
                    )
            ...
            with gr.Tab("🎙️ Dataset作成"):
                ...
                with gr.Accordion("✂️ スライス設定", open=True) as slice_accordion:
                    ...
                    with gr.Row():
                        ...
                        ds_recursive_slice = gr.Checkbox(
                            label="サブフォルダも検索", value=False, scale=1
                        )
                ...
                with gr.Accordion("🗣️ キャプション設定", open=True) as caption_accordion:
                    gr.Markdown(
                        "*faster-whisper または transformers Whisper で音声を文字起こしします。精度重視設定（large-v3 + beam=5）がデフォルトです。*"
                    )
                    ...
                    with gr.Row():
                        ...
                        ds_recursive_caption = gr.Checkbox(
                            label="サブフォルダも検索", value=False, scale=1
                        )
                ...
                with gr.Row():
                    ds_whisper_backend = gr.Dropdown(
                        label="Whisperバックエンド",
                        choices=["faster-whisper", "transformers"],
                        value="faster-whisper",
                        info="faster-whisper: 高速（GPU推奨） / transformers: 汎用的（CPU対応）",
                    )
                    ds_transformers_model = gr.Textbox(
                        label="カスタムモデル（transformers選択時）",
                        placeholder="例: openai/whisper-base",
                        value="",
                        visible=False,
                        info="transformersのモデル ID を指定（省略時は medium 相当の動作）",
                        scale=2,
                    )

                def _on_backend_change(backend):
                    is_transformers = backend == "transformers"
                    return gr.update(visible=is_transformers)

                ds_whisper_backend.change(
                    _on_backend_change,
                    inputs=[ds_whisper_backend],
                    outputs=[ds_transformers_model],
                )

                with gr.Accordion(
                    "🎭 絵文字キャプション設定（オプション）", open=False
                ) as ec_accordion:
                    gr.Markdown(
...
""")
                ...
                _ds_all_inputs = [
                    ...
                    ds_model_cache_dir,
                    ds_whisper_backend,
                    ds_transformers_model,
                ]

                _ds_exec_inputs = _ds_all_inputs + [ec_enabled, ec_api, ec_api_key]

                def _update_ds_cmd_with_custom_model(*args):
                    try:
                        # argsの構成: _ds_all_inputs + ... の内容
                ...
                for comp in _ds_all_inputs:
                    comp.change(
                        _update_ds_cmd_with_custom_model,
                        inputs=_ds_all_inputs,
                        outputs=[ds_cmd_preview],
                    )
                    ...
                    if ec_enabled_:
                        ...
                        if output_fmt != "CSV":
                            ...
                            def _wait_and_emoji(
                                ...
                                if rc == 0:
                                    _run_emoji_caption_inline(
                                        csv_path, wav_dir_, ec_api_, ec_api_key_, voice_design=vd
                                    )
            ...
            with gr.Tab("🔄 チェックポイント変換"):
                with gr.Tab("📦 通常チェックポイント変換"):
                    ...
                    conv_refresh_btn.click(
                        lambda: gr.Dropdown(
                            choices=_scan_train_checkpoints(),
                            value=(_scan_train_checkpoints() or [None])[-1],
                        ),
                        outputs=[conv_input],
                    )
            ...
            with gr.Tab("🔀 モデルマージ"):
                ...
                with gr.Accordion("⚙️ 基本マージ設定", open=True):
                    ...
                    merge_method.change(
                        _on_method_change, inputs=[merge_method], outputs=[ta_group]
                    )
                ...
                with gr.Accordion("💉 LoRA的差分注入（オプション）", open=False):
                    ...
                    with gr.Row():
                        ...
                        lora_grp_diffusion = gr.Checkbox(
                            label="diffusion_core（拡散コア）", value=False
                        )
                ...
                with gr.Tabs():
                    ...
                    with gr.Tab("🔀 通常LoRAマージ"):
                        ...
                        with gr.Row():
                            ...
                            ll_ver_a = gr.Textbox(
                                label="バージョン", interactive=False, scale=1, max_lines=1
                            )
                        ...
                        with gr.Row():
                            ll_adapter_b = gr.Dropdown(
                                label="アダプタB",
                                choices=["（なし）"] + initial_lm_adapters,
                                value=initial_lm_adapters[1]
                                if len(initial_lm_adapters) > 1
                                else "（なし）",
                                allow_custom_value=True,
                                scale=4,
                            )
                        ...
                        with gr.Accordion("⚙️ マージ設定", open=True):
                            ...
                            with gr.Group() as ll_ta_group:
                                gr.Markdown("**Task Arithmetic 設定**")
                                with gr.Row():
                                    ll_lambda_a = gr.Slider(
                                        label="λA", minimum=0.0, maximum=1.0, value=0.5, step=0.01
                                    )
                                    ll_lambda_b = gr.Slider(
                                        label="λB（自動正規化）",
                                        minimum=0.0,
                                        maximum=1.0,
                                        value=0.5,
                        ...
                        with gr.Accordion("🧩 部分マージ（グループ別手法）", open=False):
                            ...
                            ll_use_partial = gr.Checkbox(
                                label="部分マージを有効にする", value=False
                            )
                            _ll_mc = ["weighted_average", "slerp", "task_arithmetic"]
                            with gr.Group():
                                gr.Markdown("#### text グループ")
                                with gr.Row():
                                    ll_pg_text_m = gr.Dropdown(
                                        choices=_ll_mc, value="weighted_average", label="手法"
                                    )
                                    ll_pg_text_a = gr.Slider(
                                        minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="α"
                                    )
                                    ll_pg_text_la = gr.Slider(
                                        minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="λA"
                                    )
                                    ll_pg_text_lb = gr.Slider(
                                        minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="λB"
                                    )
                                gr.Markdown("#### speaker グループ")
                                with gr.Row():
                                    ll_pg_spk_m = gr.Dropdown(
                                        choices=_ll_mc, value="weighted_average", label="手法"
                                    )
                                    ll_pg_spk_a = gr.Slider(
                                        minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="α"
                                    )
                                    ll_pg_spk_la = gr.Slider(
                                        minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="λA"
                                    )
                                    ll_pg_spk_lb = gr.Slider(
                                        minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="λB"
                                    )
                                gr.Markdown("#### diffusion_core グループ")
                                with gr.Row():
                                    ll_pg_diff_m = gr.Dropdown(
                                        choices=_ll_mc, value="weighted_average", label="手法"
                                    )
                                    ll_pg_diff_a = gr.Slider(
                                        minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="α"
                                    )
                                    ll_pg_diff_la = gr.Slider(
                                        minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="λA"
                                    )
                                    ll_pg_diff_lb = gr.Slider(
                                        minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="λB"
                                    )
                                gr.Markdown("#### io グループ")
                                with gr.Row():
                                    ll_pg_io_m = gr.Dropdown(
                                        choices=_ll_mc, value="weighted_average", label="手法"
                                    )
                                    ll_pg_io_a = gr.Slider(
                                        minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="α"
                                    )
                                    ll_pg_io_la = gr.Slider(
                                        minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="λA"
                                    )
                                    ll_pg_io_lb = gr.Slider(
                                        minimum=0.0, maximum=1.0, value=0.5, step=0.01, label="λB"
                                    )
                        ...
                        with gr.Accordion("💾 出力設定", open=True):
                            ll_output_dir = gr.Textbox(
                                ...
                                value="",
                                placeholder=str(
                                    Path(__file__).resolve().parent / "lora" / "lora_merged_*"
                                ),
                            )
                        ...
                        ll_method.change(
                            _ll_method_change, inputs=[ll_method], outputs=[ll_ta_group]
                        )
                    ...
                    with gr.Tab("🔥 本体モデルマージ（焼き込み）"):
                        ...
                        with gr.Row():
                            lm_base_model = gr.Dropdown(
                                ...
                                value=default_lm_base
                                if default_lm_base in initial_lm_ckpts
                                else (initial_lm_ckpts[-1] if initial_lm_ckpts else None),
                                allow_custom_value=True,
                                scale=4,
                            )
                        ...
                        with gr.Accordion("🅰️ アダプタA（必須）", open=True):
                            with gr.Row():
                                lm_adapter_a1 = gr.Dropdown(
                                    label="アダプタA-1",
                                    choices=["（なし）"] + initial_lm_adapters,
                                    value=initial_lm_adapters[0]
                                    if initial_lm_adapters
                                    else "（なし）",
                                    allow_custom_value=True,
                                    scale=4,
                                )
                                lm_scale_a1 = gr.Slider(
                                    label="scale",
                                    minimum=0.0,
                                    maximum=2.0,
                                    value=1.0,
                        ...
                        with gr.Accordion("🔀 焼き込み後マージ（オプション）", open=False):
                            ...
                            with gr.Group() as lm_post_group:
                                ...
                                with gr.Row():
                                    lm_post_lam_a = gr.Slider(
                                        label="λA", minimum=0.0, maximum=1.0, value=0.5, step=0.01
                                    )
                                    lm_post_lam_b = gr.Slider(
                                        label="λB", minimum=0.0, maximum=1.0, value=0.5, step=0.01
                                    )
                                with gr.Row():
                            ...
                            with gr.Accordion(
                                ...
                                with gr.Group():
                                    gr.Markdown("#### text グループ")
                                    with gr.Row():
                                        lm_pg_text_m = gr.Dropdown(
                                            choices=_lm_mc, value="weighted_average", label="手法"
                                        )
                                        lm_pg_text_a = gr.Slider(
                                            minimum=0.0,
                                            maximum=1.0,
                                            value=0.5,
                                            step=0.01,
                                    ...
                                    gr.Markdown("#### speaker グループ")
                                    with gr.Row():
                                        lm_pg_spk_m = gr.Dropdown(
                                            choices=_lm_mc, value="weighted_average", label="手法"
                                        )
                                        lm_pg_spk_a = gr.Slider(
                                            minimum=0.0,
                                            maximum=1.0,
                                            value=0.5,
                                            step=0.01,
                                    ...
                                    gr.Markdown("#### diffusion_core グループ")
                                    with gr.Row():
                                        lm_pg_diff_m = gr.Dropdown(
                                            choices=_lm_mc, value="weighted_average", label="手法"
                                        )
                                        lm_pg_diff_a = gr.Slider(
                                            minimum=0.0,
                                            maximum=1.0,
                                            value=0.5,
                                            step=0.01,
                                    ...
                                    gr.Markdown("#### io グループ")
                                    with gr.Row():
                                        lm_pg_io_m = gr.Dropdown(
                                            choices=_lm_mc, value="weighted_average", label="手法"
                                        )
                                        lm_pg_io_a = gr.Slider(
                                            minimum=0.0,
                                            maximum=1.0,
                                            value=0.5,
                                            step=0.01,
                        ...
                        lm_run_btn = gr.Button(
                            "🔥 焼き込みマージ実行", variant="primary", size="lg"
                        )
                        ...
                        lm_post_method.change(
                            _lm_post_method_change,
                            inputs=[lm_post_method],
                        ...
                        def _run_lm_bake_ui(
                            ...
                            for ad, sc in [(adapter_a1, scale_a1), (adapter_a2, scale_a2)]:
                                if str(ad).strip() not in ("", "（なし）"):
                                    dirs_a.append(str(ad).strip())
                                    scales_a.append(float(sc))
                            if not dirs_a:
                            ...
                            for ad, sc in [(adapter_b1, scale_b1), (adapter_b2, scale_b2)]:
                                if str(ad).strip() not in ("", "（なし）"):
                                    dirs_b.append(str(ad).strip())
                                    scales_b.append(float(sc))

                            gb_a = (
                                {
                                    "text": bool(pbake_a_text),
                                    "speaker": bool(pbake_a_spk),
                                    "diffusion_core": bool(pbake_a_diff),
                                    "io": bool(pbake_a_io),
                                }
                                if use_pbake_a
                                else None
                            ...
                            gb_b = (
                                {
                                    "text": bool(pbake_b_text),
                                    "speaker": bool(pbake_b_spk),
                                    "diffusion_core": bool(pbake_b_diff),
                                    "io": bool(pbake_b_io),
                                }
                                if use_pbake_b
                                else None
                            ...
                            _, msg = run_lora_merge(
                                ...
                                adapter_dirs_a=dirs_a,
                                adapter_scales_a=scales_a,
                                ...
                                post_lambda_a=lam_a_n,
                                post_lambda_b=lam_b_n,
                                post_base_path=pb,
                                use_partial=bool(use_partial),
                                group_methods=gm,
                                use_partial_bake_a=bool(use_pbake_a),
                                group_bake_a=gb_a,
                                use_partial_bake_b=bool(use_pbake_b),
                                group_bake_b=gb_b,
