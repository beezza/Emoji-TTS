...
> 📚 **詳細ドキュメント**: [DATASET_TOOLS.md](docs/DATASET_TOOLS.md) / [WHISPER_BACKENDS.md](docs/WHISPER_BACKENDS.md) / [QUICK_START.md](docs/QUICK_START.md)
...
**🆕 新機能**: `--whisper-backend` オプションで `faster-whisper`（推奨）または `transformers` を選択可能。詳細は [WHISPER_BACKENDS.md](docs/WHISPER_BACKENDS.md) を参照。

| パラメータ | 説明 |
|---|---|
| Whisperモデル | `tiny` / `base` / `small` / `medium` / `large-v3` |
| Whisperバックエンド | `faster-whisper`（推奨、高速、NVIDIA GPU必須）または `transformers`（汎用、CPU対応） |
